...
// 1. Словарь переводов (Будем пополнять)
window.i18n = {
    ru: {
        'search_placeholder': 'Я ищу...',
        'btn_add': 'Пристроить добро',
        'btn_add_mobile': 'Продать',
        'nav_login': 'Войти',
        'nav_info': 'Инфо',
        'feed_title': 'Новые находки',
        'feed_desc': 'Самые свежие объявления Сербии',
    },
    en: {
        ...
        'nav_login': 'Login',
        'nav_info': 'Info',
        'feed_title': 'New Arrivals',
        'feed_desc': 'Latest classifieds in Serbia',
    },
    sr: {
        'search_placeholder': 'Tražim...',
        'btn_add': 'Objavi oglas',
        'btn_add_mobile': 'Prodaj',
        'nav_login': 'Prijavi se',
        'nav_info': 'Info',
        'feed_title': 'Novi oglasi',
        'feed_desc': 'Najnoviji oglasi u Srbiji',
    }
};

// 2. Функция для перевода динамического контента (JS)
window.t = (key) => {
    return window.i18n[window.currentLang][key] || key;
};

// 3. Основная функция переключения (без перезагрузки)
window.changeLanguage = (lang) => {
    window.currentLang = lang;
    localStorage.setItem('svalka_lang', lang); // Запоминаем выбор юзера
    
    // Обновляем иконки во всех переключателях на сайте
    document.querySelectorAll('.lang-flag').forEach(el => {
        if(lang === 'ru') el.innerText = '🇷🇺';
        if(lang === 'sr') el.innerText = '🇷🇸';
        if(lang === 'en') el.innerText = '🇬🇧';
    });
    
    document.querySelectorAll('.lang-code').forEach(el => {
        if(lang === 'ru') el.innerText = 'RU';
        if(lang === 'sr') el.innerText = 'SR';
        if(lang === 'en') el.innerText = 'EN';
    });

    // Находим все элементы с атрибутом data-i18n и меняем текст "на лету"
