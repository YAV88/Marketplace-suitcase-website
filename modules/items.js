...
export const ItemsModule = {
    ...
    createCardHtml: (i, isVIP, isProfileView = false) => {
        ...
        const vipCardClasses = 'vip-card border-2 border-amber-400/60 dark:border-amber-500/60 shadow-xl shadow-amber-500/20 dark:shadow-amber-500/20 hover:shadow-amber-500/30';
        ...
        if (i.status === 'reserved') { 
            statusBadgeRow = `<span class="bg-orange-50 text-orange-600 border border-orange-200 dark:bg-orange-900/30 dark:border-orange-800 text-[10px] font-black px-2 py-0.5 rounded shadow-sm tracking-widest uppercase flex items-center shrink-0">${t('В РЕЗЕРВЕ')}</span>`; 
        }
            ...
            statusBadgeRow = `<span class="bg-stone-100 text-stone-600 border border-stone-200 dark:bg-stone-800 dark:text-stone-300 text-[10px] font-black px-2 py-0.5 rounded shadow-sm tracking-widest uppercase flex items-center shrink-0">${t('ПРОДАНО')}</span>`;
            ...
            if (!isLoadMore && vipRes.data && !window.showUrgentOnly) {
                ...
                if (initialVips.length > 0 && vipGrid && vipSection) {
                    ...
                    vipGrid.innerHTML = initialVips.map((i, idx) => {
                        // Оборачиваем каждую карточку в div-слот для логики ротации
                        return `<div class="vip-slot transition-all duration-700 h-full flex transform-gpu" data-slot="${idx}">${ItemsModule.createCardHtml(i, true)}</div>`;
                    }).join('');
    ...
    openItemDetails: async (id) => {
        ...
        try {
           ...
           if (carousel) {
            ...
            const condTextEl = document.getElementById('modal-condition-badge');
            if (condTextEl) {
                if (isService || isJob || isEstate || isAnimalEntity) { 
                    condTextEl.classList.add('hidden');
                    condTextEl.classList.remove('flex');
                } else { 
                    // Принудительно перебиваем старый HTML и задаем ТЕ ЖЕ классы, что и у других плашек!
                    condTextEl.className = 'flex items-center justify-center bg-stone-100 dark:bg-stone-800 text-stone-700 dark:text-stone-300 text-xs font-bold px-3 py-1.5 rounded-lg border border-stone-200 dark:border-stone-700 shrink-0 w-max transition-colors';
                    
                    condTextEl.innerHTML = item.condition === 'Новое' 
                        ? '<i class="fa-solid fa-sparkles text-amber-500 text-[13px] mr-1.5"></i> Новое' 
                        : '<i class="fa-solid fa-recycle text-[13px] opacity-80 mr-1.5 relative -top-[1px]"></i> Б/У';
                }
            }
            ...
            if (addrCont) {
                ...
                if (needsMap && item.coords && Array.isArray(item.coords) && item.coords.length === 2 && item.coords[0] !== 0) {
                    ...
                    // Если это обычный товар — прячем весь блок карты
            ...
            if (authorSubEl) {
                ...
                authorSubEl.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> Проверка продавца...';

                // Запрашиваем данные продавца из profiles и reviews одновременно (Параллельно = Быстро)
                Promise.all([
                    supabase.from('profiles').select('pro_until, is_verified').eq('id', item.userId).single(),
                    supabase.from('reviews').select('rating').eq('seller_id', item.userId)
                    ...
                    if (reviewsData && reviewsData.length > 0) {
                        ...
                        badgesHtml += `<span class="flex items-center gap-1 text-stone-800 dark:text-stone-200 bg-white dark:bg-stone-800 px-1.5 py-0.5 rounded-md shadow-sm border border-stone-200 dark:border-stone-700"><i class="fa-solid fa-star text-amber-500 text-[10px]"></i> <span class="font-black">${avg}</span> <span class="text-stone-400 font-medium ml-0.5">(${reviewsData.length})</span></span>`;
                    }

                    authorSubEl.innerHTML = badgesHtml;
                }).catch((e) => {
                    console.error("Ошибка проверки продавца:", e);
                    authorSubEl.innerHTML = `<span class="shrink-0">Продавец SVALKA</span>`;
                });
            }
